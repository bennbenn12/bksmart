    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-3">
          {cart.map(item => (
            <div key={item.item_id} className="bg-white rounded-xl border border-slate-100 p-4 flex items-center gap-4 hover:border-slate-200 transition-colors shadow-sm">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400 mt-0.5">{item.category} · {item.shop?.replace('_',' ')}</p>
                <p className="font-bold text-hnu-dark mt-1">{formatCurrency(item.price)}</p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <button onClick={()=>updateQuantity(item.item_id, item.quantity-1)} className="w-7 h-7 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center transition-colors"><Minus size={12}/></button>
                <span className="w-8 text-center font-bold text-slate-700">{item.quantity}</span>
                <button onClick={()=>updateQuantity(item.item_id, item.quantity+1)} className="w-7 h-7 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center transition-colors"><Plus size={12}/></button>
              </div>
        ...
        <div className="space-y-4">
          ...
          <Link href="/shop/checkout" className="block w-full btn-primary py-3 justify-center text-base font-bold shadow-lg shadow-brand-500/20 text-center">
            Place Order Slip <ArrowRight size={16}/>
          </Link>
