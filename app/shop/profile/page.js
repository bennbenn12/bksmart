...
import { Package, User, Calendar, ListOrdered, Bell, ChevronRight } from 'lucide-react'
...
export default async function ProfilePage() {
  ...
  const { data: profile } = await supabase
    .from('users')
    .select('user_id, first_name, last_name, email, student_id, contact_number, role_id, created_at')
    .eq('auth_id', user.id)
    .single()
  ...
  const [
    ...
    supabase
      .from('appointments')
      .select('appointment_id, schedule_date, time_slot, status, order:order_id(order_number)')
      .eq('user_id', profile.user_id)
    ...
    <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="flex items-end gap-4 -mt-8 mb-4">
            ...
            <div className="pb-1">
              ...
              <span className="text-xs font-semibold bg-brand-100 text-brand-700 px-2 py-0.5 rounded-full">
                {ROLE_LABELS[profile.role_id] || profile.role_id}
              </span>
            </div>
            <div className="ml-auto pb-1"><SignOutButton /></div>
          </div>
      ...
      {activeQueue && (
        <div className={cn(
          ...
          activeQueue.status === 'Processing' ? 'border-green-400 bg-green-50' : 'border-brand-300 bg-brand-50'
        )}>
          <div className={cn(
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          ...
          {!appointments?.length ? (
            ...
            <div key={a.appointment_id} className="flex items-center gap-3 px-5 py-4 border-t border-slate-50">
